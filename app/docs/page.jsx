    ...
    <div className="flex justify-between m-auto w-[1440px] mt-[100px]">
        ...
        <div className="flex">
          ...
          <div>
            {descriptionSections.sort().map((item) => {
              return <p className="text-md mb-2 hover:scale-125 hover:cursor-pointer origin-left  duration-200 ease-in-out"><a href={`#${item.id}`}>{item.title}</a></p>;
            })}
          </div>
        </div>
      </div>
      <div className="w-full ">
        {descriptionSections.map((item) => (
          <div className="scroll-pt-5" id={item.id}>
